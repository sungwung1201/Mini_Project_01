...
const NAV_ITEMS: NavItem[] = [
  { key: 'dashboard', label: '대시보드', icon: IconLayoutDashboard, roles: 'all' },
  { key: 'students', label: '학생 등록', icon: IconUserPlus, roles: ['admin'] },
  { key: 'attendance', label: '출결 입력', icon: IconCalendarCheck, roles: 'all' },
  { key: 'grades', label: '평가/점수', icon: IconBarChart, roles: 'all' },
  { key: 'courses', label: '과정 관리', icon: IconBookOpen, roles: ['admin'] },
  { key: 'assignments', label: '과제 목록', icon: IconClipboardList, roles: 'all' },
];
...
function Topbar({ tab, setTab, role }: { tab: Tab; setTab: (t: Tab) => void; role: Role }) {
  const items = visibleNavItems(role).filter((i) => i.key !== 'dashboard');

  return (
    <div className="topbar">
      <div className="topbar-main">
        <div className="topbar-title">업무 탭</div>
        <div className="topbar-chips" role="tablist" aria-label="업무 탭">
          {items.map((item) => {
            const Icon = item.icon;
...
function GradeSection({ notify }: { notify: (type: Toast['type'], msg: string) => void }) {
  ...
  const exportStudentGrades = () => {
    if (!studentGrade || studentGrade.length === 0) return notify('error', '학생 성적 데이터가 없습니다.');
