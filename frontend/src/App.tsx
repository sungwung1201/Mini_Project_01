...
type Tab = 'dashboard' | 'students' | 'attendance' | 'grades' | 'courses' | 'assignments';
...
type NavTabItem = { key: Tab; label: string; icon: SvgIcon; roles: Role[] | 'all' };

const NAV_TABS: NavTabItem[] = [
  { key: 'students', label: '학생등록', icon: IconUserPlus, roles: ['admin'] },
  { key: 'attendance', label: '출결입력', icon: IconCalendarCheck, roles: 'all' },
  { key: 'grades', label: '평가/점수', icon: IconBarChart, roles: 'all' },
...
function tabsForRole(role: Role) {
  return NAV_TABS.filter((t) => t.roles === 'all' || t.roles.includes(role));
}
  ...
  return (
    <div className="header-bar">
      ...
      <div className="header-actions">
        <select value={role} onChange={(e) => onRoleChange(e.target.value as Role)} className="select">
...
function TabBar({ tab, setTab, role }: { tab: Tab; setTab: (t: Tab) => void; role: Role }) {
...
function DashboardSection({ go, role }: { go: (tab: Tab) => void; role: Role }) {
    ...
    <div className="dashboard">
      <div className="hero">
        <div>
          ...
          <h1>오늘의 학사 현황</h1>
          <p className="muted">탭을 눌러 학생등록, 출결입력, 평가/점수, 과정관리, 과제목록으로 이동합니다.</p>
        </div>
        <div className="hero-actions">
          <button className="secondary" onClick={() => go('attendance')} type="button">
            출결입력
          </button>
          {role === 'admin' && (
            <button onClick={() => go('students')} type="button">
              학생등록
            </button>
          )}
        </div>
      ...
      <div className="card">
        <div className="header">
          <h3 style={{ margin: 0 }}>과정 선택</h3>
        </div>
        <div className="form-row">
          <div>
            <label>과정</label>
            <select value={courseId ?? ''} onChange={(e) => setSelectedCourseId(Number(e.target.value) || null)} className="select">
      ...
      <div className="grid two">
        <div className="card">
          ...
          {recentStudents.length === 0 ? (
            <small className="muted">학생 데이터가 없습니다.</small>
          ) : (
            <ul className="list">
        ...
        <div className="card">
          ...
          {recentCourses.length === 0 ? (
            <small className="muted">과정 데이터가 없습니다.</small>
          ) : (
            <ul className="list">
              {recentCourses.map((c) => (
...
function StudentsSection({ notify }: { notify: (type: Toast['type'], message: string) => void }) {
  ...
  return (
    <div className="card">
      <div className="header">
        <h2 className="section-title">학생등록</h2>
      ...
      <button className="btn-slim" onClick={addStudent} type="button">
        학생 추가
      </button>
      ...
      <div className="pager">
        <button className="secondary" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} type="button">
          이전
        </button>
        <span className="muted" style={{ fontSize: 12 }}>
          {page} / {totalPages}
        </span>
        <button className="secondary" disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))} type="button">
          다음
        </button>
      </div>
    </div>
...
function CoursesSection({ notify }: { notify: (type: Toast['type'], message: string) => void }) {
  ...
  return (
    <div className="card">
      <h2 className="section-title">과정관리</h2>
      ...
      <button className="btn-slim" onClick={addCourse} type="button">
        과정 추가
      </button>
...
function AttendanceSection({ notify }: { notify: (type: Toast['type'], message: string) => void }) {
  ...
  return (
    <div className="card">
      <h2 className="section-title">출결입력</h2>

      <div className="form-row">
        <div>
          <label>과정 선택</label>
          <select value={courseId || ''} onChange={(e) => setSelectedCourse(Number(e.target.value) || null)} className="select">
            {(courses || []).map((c) => (
              <option key={c.id} value={c.id}>
                {c.name}
              </option>
        ...
        <div>
          <label>회차 선택</label>
          <select value={selectedSession || ''} onChange={(e) => setSelectedSession(Number(e.target.value) || null)} className="select">
      ...
      <div className="header">
        <button className="secondary" onClick={createSession} type="button">
          회차 생성
        </button>
        <div className="inline">
          <select value={bulkStatus} onChange={(e) => setBulkStatus(e.target.value as any)} className="select" style={{ maxWidth: 140 }}>
          ...
          <button onClick={applyBulk} type="button">
            전체 적용
          </button>
        </div>
      </div>
          ...
          <table className="table">
            ...
            <tbody>
              {(records || []).map((r) => (
                <tr key={r.id}>
                  <td>{r.student_id}</td>
                  <td>{studentNameById.get(r.student_id) || `학생 #${r.student_id}`}</td>
                  <td>
                    <select value={r.status} onChange={(e) => updateRecord(r.student_id, e.target.value as any)} className="select">
                      {attendanceOptions.map((o) => (
                        <option key={o.value} value={o.value}>
                          {o.label}
                        </option>
...
function AssignmentsSection() {
  ...
  return (
    <div className="card">
      <h2 className="section-title">과제목록</h2>
      <div className="form-row">
        <div>
          <label>과정 선택</label>
          <select value={courseId || ''} onChange={(e) => setSelectedCourse(Number(e.target.value) || null)} className="select">
      ...
      <p className="muted">평가는 평가/점수 탭에서 생성 및 점수 입력을 진행합니다. 여기서는 목록 확인 중심입니다.</p>
...
function GradesSection({ notify }: { notify: (type: Toast['type'], message: string) => void }) {
  ...
  const [assessmentForm, setAssessmentForm] = useState({ name: '', weight: 1, max_score: 100 });
  ...
  const exportCourseGrade = () => {
    ...
    const body = [String(courseGrade.course_id), String(courseGrade.course_name), String(courseGrade.average_score ?? '')];
  ...
  const addAssessment = async () => {
    if (!courseId) return;
    const payload = { name: assessmentForm.name, weight: assessmentForm.weight, max_score: assessmentForm.max_score };
    const res = await api.post<Assessment>(`/courses/${courseId}/assessments`, payload);
  ...
  const downloadScoresCSV = (assessmentId: number) => {
    ...
    const rows: string[][] = [
      ...
      ...list.map((s) => [String(s.student_id), studentNameById.get(s.student_id) || `학생 #${s.student_id}`, String(s.raw_score)]),
    ];
    const csv = rows.map((r) => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
  ...
  return (
    <div className="card">
      <h2 className="section-title">평가/점수</h2>
      ...
      <div className="form-row">
        ...
        <div>
          <label>학생 선택</label>
          <select value={studentId || ''} onChange={(e) => setSelectedStudent(Number(e.target.value) || null)} className="select">
            {(students || []).map((s) => (
              <option key={s.id} value={s.id}>
                {s.full_name}
              </option>
        ...
        <div className="row-right">
          <button className="btn-compact secondary" onClick={exportCourseGrade} disabled={!courseGrade} type="button">
            강좌 요약 CSV
          </button>
          <button className="btn-compact" onClick={exportStudentGrades} disabled={!studentGrade} type="button">
            학생 성적 CSV
          </button>
        </div>
      </div>
      ...
      <div className="grid two">
        <div className="card subcard">
          ...
          <div className="form-row">
            ...
            <div>
              <label>만점</label>
              <input type="number" value={assessmentForm.max_score} onChange={(e) => setAssessmentForm({ ...assessmentForm, max_score: Number(e.target.value) })} />
            </div>
          </div>
          <button className="btn-slim" onClick={addAssessment} disabled={!courseId} type="button">
            평가 추가
          </button>
        </div>
        ...
        <div className="card subcard">
          ...
          {!studentGrade || studentGrade.length === 0 ? (
            <small className="muted">데이터가 없습니다.</small>
          ) : (
            <table className="table">
              <thead>
                <tr>
      ...
      <div className="card subcard" style={{ marginTop: 14 }}>
        <div className="header">
          <h3 style={{ margin: 0 }}>평가 목록</h3>
        </div>
        {!assessments || assessments.length === 0 ? (
          <small className="muted">평가가 없습니다.</small>
        ) : (
          <table className="table">
            <thead>
              <tr>
                ...
                <tr key={a.id}>
                  ...
                  <td style={{ minWidth: 320 }}>
                    <div className="score-row">
                      ...
                      <button className="secondary" onClick={() => saveScores(a.id)} type="button">
                        저장
                      </button>
                    </div>
                  </td>
                  <td>
                    <button className="secondary" onClick={() => downloadScoresCSV(a.id)} type="button">
                      점수 CSV
                    </button>
                  </td>
                </tr>
...
export default function App() {
  ...
  useEffect(() => {
    const items = tabsForRole(role);
    if (tab !== 'dashboard' && !items.find((i) => i.key === tab)) {
      setTab(items[0].key);
    }
  }, [role, tab]);

  if (!loggedIn) return <LoginCard onSuccess={() => setLoggedIn(true)} />;

  return (
    <div className="app">
      <HeaderBar tab={tab} onGoHome={() => setTab('dashboard')} role={role} onRoleChange={setRole} onLogout={logout} />
      <TabBar tab={tab} setTab={setTab} role={role} />

      <main className="main">
        {tab === 'dashboard' && <DashboardSection go={setTab} role={role} />}
        {tab === 'students' && role === 'admin' && <StudentsSection notify={pushToast} />}
        {tab === 'courses' && role === 'admin' && <CoursesSection notify={pushToast} />}
        {tab === 'attendance' && <AttendanceSection notify={pushToast} />}
        {tab === 'grades' && <GradesSection notify={pushToast} />}
